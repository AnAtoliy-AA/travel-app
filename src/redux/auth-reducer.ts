...
const authReducer = (state = initialState, action: { type: string; isAuthorized: boolean; token: string }) => {
    switch (action.type) {
...
export const setUserData = (token: string) => ({ type: ACTION_CONST.SET_USER_DATA, token });
